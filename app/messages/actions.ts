...
function buildManagerAssignedMessage(manager: {
  ...
  // Сервисное сообщение для клиента: отправляем его в момент,
  // когда обращение передали конкретному специалисту.
  return [
    "Здравствуйте! Ваш вопрос уже передан нашему специалисту.",
    "",
    `С вами будет работать ${manager.name} ${manager.surname}, ${manager.position}.`,
